...
open class Highlightr {
    ...
    /**
     Format the code by enveloping keywords, strings, values, etc., in html tags.
     
     - parameter code:           Code to highlight.
     - parameter languageName:   Language name or alias. Set to `nil` to use auto detection.
     */
    open func formatHtml(_ code: String, as languageName: String? = nil) -> String? {
        let ret: JSValue
    ...
    /**
     Renders the provided html by applying the specified theme.
     
     - Parameter html: Formatted html to be rendered
     ...
     */
    open func render(_ html: String, fastRender: Bool = true) -> NSAttributedString? {
        var html = html
        var returnString : NSAttributedString?
        if fastRender {
            returnString = processHTMLString(html)!
        } else {
            html = "<style>"+theme.lightTheme+"</style><pre><code class=\"hljs\">"+html+"</code></pre>"
            ...
            safeMainSync {
                returnString = try? NSMutableAttributedString(
                    data: data,
                    options: opt,
    ...
    open func highlight(_ code: String, as languageName: String? = nil, fastRender: Bool = true) -> NSAttributedString? {
        guard let htmlStr = formatHtml(code, as: languageName) else {
            return nil
        }
        return render(htmlStr, fastRender: fastRender)
    }
